...
from project.task1 import get_graph, get_graph_info
from project.task3 import regular_path_querying
...
def test_get_reachable(inp, graph, start, final):
    ...
    gr = get_graph(graph)
    labels = get_graph_info(graph).labels
    regex = "(" + labels.pop()
    for label in labels:
        regex += "|" + label
    regex += ")*"
    regex = Regex(regex)
    exp = set()
    for (_, j) in regular_path_querying(
        graph=gr, regex=regex, start_states=start, final_states=final
    ):
        exp.add(j)
    assert vertices == exp
