...
from project.task3 import regular_path_querying_for_interpreter
...
class Visitor(GQLVisitor):
    ...
    def visitStmt(self, ctx: GQLParser.StmtContext):
        if ctx.var():
            name = ctx.var().IDENTIFIER_CHAR().getText()
    ...
    def visitVar(self, ctx: GQLParser.VarContext):
        name = ctx.IDENTIFIER_CHAR().getText()
    ...
    def visitPath(self, ctx: GQLParser.PathContext):
        if ctx.var():
            ...
            try:
                return create_nfa_for_graph(
                    graph=get_graph(name), start_states=set(), final_states=set()
                )
            except:
    ...
    def visitVertices(self, ctx: GQLParser.VerticesContext):
        if ctx.var():
            ...
            graph = self.visit(ctx.graph())
            return regular_path_querying_for_interpreter(graph)
